...
- **Router — three routes.** `src/app/app.routes.ts`: `/sign-in` → `SignInComponent` behind `signInGuard` (`S-01`), `''` → `ShellComponent` behind `authGuard` (`S-01`), and `/students` → `StudentRosterComponent` behind `authGuard` (`S-03`), with `**` redirecting to `''`. New routes go **before** the wildcard. `provideRouter(routes)` sits in `app.config.ts`. `AppComponent` is now just the header plus a `<router-outlet>` plus the boot indicator. There is deliberately **no route per report type** — adding one would rewrite the registry below for no gain. `/students` is a route rather than a tab precisely because of the split above: a roster is neither a report nor a tab.
...
**Leaving the shell resets the active tab and drops unsaved report-form state, and that is inherent to the split above, not a bug.** `ShellComponent` is a routed component: navigating to `/students` destroys it, and coming back re-instantiates it with `TabData.tabs` reset to its `defaultActive` entry (Teddy Eddie) and every report form blank. `S-03` accepted this — managing a roster is something a teacher does *before* writing reports.

**`S-04` closed the open question that paragraph left, and it closed it with quick-add, not with preserved state.** The mid-report case — "this student isn't in my roster yet" — is answered by the quick-add area inside `StudentPickerComponent`: the teacher creates the student from within the report form, the new student is selected immediately, and the form is never destroyed because no navigation happens. The shell reset itself is **unchanged**: leaving the shell still drops the form, and that is still not a bug. What changed is that the roster case no longer needs you to leave. Do not read this as an invitation to add a `RouteReuseStrategy` or preserved shell state — that refactor was considered and deliberately declined, and any future surface with the same problem should get the same in-place treatment first.
...
`src/app/students/` (`S-03`) is the second instance of that seam and the one to read if you want the pattern without the template-specific domain logic: `students.gateway.ts` (every SDK call, no decisions), `students.service.ts` (validation, the uid-gated signal cache, tolerant reads, error classification), `student-roster/` (the one component). Two things differ from templates and both are deliberate: **`students` is the project's first MUTABLE collection** — its rule allows `update`, where `reportTemplates` denies it to everyone including the owner (PRD §Non-Goals makes templates write-once; FR-007 makes editing a student first-class) — and **its document ids are Firestore auto-ids, not the normalized name**, because a student's name is mutable and non-unique, so a name-keyed id would turn a rename into a non-atomic delete + create. `firestore.rules` carries the full reasoning at the rule itself.
...
**One writer changes `PER_STUDENT_FIELDS` values without belonging to any panel's domain: the sex-driven mark remap** (`S-04`). The six descriptive-mark selects build their option lists from `mark.value` or `mark.valueFemale` depending on `sex` (`markOptions()`), so a control still holding the other variant matches no option — the select renders blank, `required` still passes because the control holds a value, and the wrong-gender sentence reaches the PDF. `SemestrReportComponent` subscribes to `form.controls['sex'].valueChanges` and rewrites those six controls through `helper/marks/sex-variant.ts`. Two properties to preserve: it is keyed on **`sex`, not on the picker**, so a manual change of the sex select behaves identically and this is a property of the form rather than a second pre-fill mechanism; and it changes *values* inside `PER_STUDENT_FIELDS` without moving any field across the partition, so it is not a widening of the picker's domain. It subscribes to the **child** control's stream rather than the group's on purpose — `patchValue` writes children with `onlySelf: true`, so the remap runs mid-patch and the invariant that makes that safe is that it reads `sex` and the six mark controls and nothing else.
...
- `src/app/templates/` — the FR-009…FR-012 report-template surface: `template-domain.ts` (the four-set partition of the trimester/semester form's controls, plus the domain defaults and `schemaVersion`), `templates.gateway.ts` (the only file calling Firestore for templates), `templates.service.ts` (every decision), `template-panel/` (the panel mounted at the top of the trimester/semester form) and `confirm-dialog/` (the app's only `MatDialog` — generic on purpose — `S-03` reuses it unchanged for "delete a student", so keep it content-free).
- `src/app/students/` — the FR-005…FR-008 per-teacher roster plus FR-013's picker: `student-domain.ts` (the four identity field names and their defaults, plus `schemaVersion`; a spec asserts it names the same four controls as `templates/template-domain.ts`'s `STUDENT_IDENTITY_FIELDS`, which is the seam the picker relies on), `students.gateway.ts` (the only file calling Firestore for students), `students.service.ts` (every decision), `student-failure-keys.ts` (`StudentsFailure` → translate key, total over the union so a new failure mode is a compile error, plus the form-shaped subset), `student-identity-diff.ts` (`diffIdentity` — what a pick would overwrite or clear, a pure function so it unit-tests like `TemplatesService.diff`; `null` and `''` both count as default), `student-form/` (the four fields and their inline error line, mounted by **both** the roster and the picker's quick-add, and the one definition of `StudentFormControls` / `createStudentForm` / `readStudentForm` / `resetStudentForm`), `student-roster/` (the `/students` route — add, list, edit, delete, reusing `templates/confirm-dialog/` for the delete prompt), and `student-picker/` (the panel at the top of the trimester/semester form — pick, confirm-before-overwrite, quick-add).
...
- `src/app/helper/` — pure-TypeScript helpers, no Angular decorators: `cambridge/` (the exam-table generators) and `marks/sex-variant.ts` (`counterpartValue` — the same mark sentence in the other gender's variant, matching on both `value` and `valueFemale` so the function is its own inverse, and returning an unmatched value untouched rather than blanking it). New pure helpers go here, not in `shared/`.
- `src/app/model/` — TypeScript interfaces and types only. No runtime code (`development-path-in-school.ts`, `development-path-teddy-eddie.ts`, `tab.interface.ts`, `auth.interface.ts`, `student.interface.ts`).
...
**Every file in `test/rules/` needs its own `projectId`, and adding one without it breaks a file you did not touch.** `node --test test/rules` starts one process per file, in parallel, against a single shared emulator, and `@firebase/rules-unit-testing` namespaces both data and rules by project id — so two files sharing an id share a dataset, and each one's `clearFirestore()` in `afterEach` deletes the other's fixtures mid-test. `S-03` found this the expensive way: adding `students.test.mjs` made a stable, untouched `report-templates.test.mjs` fail intermittently on a create its own `seedAllowlist` had just made legal, and the error (`PERMISSION_DENIED`, `evaluation error at L…`) pointed at the rules rather than at the runner. A third file needs a third id.
