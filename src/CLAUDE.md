...
**Leaving the shell resets the active tab and drops unsaved report-form state, and that is inherent to the split above, not a bug.** `ShellComponent` is a routed component: navigating to `/students` destroys it, and coming back re-instantiates it with `TabData.tabs` reset to its `defaultActive` entry (Teddy Eddie) and every report form blank. `S-03` accepted this — managing a roster is something a teacher does *before* writing reports. It stops being acceptable in `S-04`, where "this student isn't in my roster yet" arrives mid-report; that slice needs either an in-form quick-add or preserved form state. Do not "fix" it as a drive-by — the fix is a design decision, not a refactor.
...
- `src/app/students/` — the FR-005…FR-008 per-teacher roster, mounted at `/students`: `student-domain.ts` (the four identity field names and their defaults, plus `schemaVersion`; a spec asserts it names the same four controls as `templates/template-domain.ts`'s `STUDENT_IDENTITY_FIELDS`, which is the seam `S-04`'s picker relies on), `students.gateway.ts` (the only file calling Firestore for students), `students.service.ts` (every decision), and `student-roster/` (the whole teacher-facing surface — add, list, edit, delete, reusing `templates/confirm-dialog/` for the delete prompt).
