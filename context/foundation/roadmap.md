...
| S-05a | `report-design-language`     | (enabling) the visual language is extracted from the Teddy Eddie form into tokens, mixins, and shared patterns, and written down | F-01, F-02    | OQ-1                                              | merged, **deployed 2026-07-30** |
| S-05b | `report-design-refresh`      | fill the semester, year-end, and Cambridge **forms** in that same visual language | S-05a         | OQ-1, FR-014, FR-015, FR-016, FR-017              | done, **deployed 2026-07-30** |
| S-01 | `google-sign-in-gate`         | sign in with Google and reach the four existing report forms; unauthenticated visitors cannot | F-01, F-02    | FR-001, FR-002, FR-003, FR-004, FR-014, FR-015, FR-016, FR-017, FR-018 | done, **deployed 2026-07-30** |
| S-02 | `trimester-report-templates`  | save a trimester/semester report as a named template, list, apply, and delete templates | S-01          | US-01, FR-009, FR-010, FR-011, FR-012, FR-014, FR-018 | done, **deployed 2026-07-30** |
| S-03 | `student-roster`              | add, view, edit, and delete students on their own roster                       | S-01          | FR-005, FR-006, FR-007, FR-008, FR-018            | done, **deployed 2026-07-31** |
...
- `context/foundation/infrastructure.md` selects **Firebase (Hosting + Authentication + Firestore)** as the platform, and records **`eur3` (Europe multi-region)** as the Firestore location. This closed PRD Open Question #3, which the PRD now also lists as resolved (agreement reached 2026-07-30 — see Open Roadmap Question #3).
...
- **Status:** merged into `10xdevs-m2l5`, **deployed 2026-07-30** as part of `S-02`'s Phase 6
...
- **Status:** done — archived 2026-07-29, **deployed 2026-07-30** as part of `S-02`'s Phase 6
...
- **Outcome (2026-07-28):** delivered in five phases on `feature/google-sign-in-gate` (`ca0458e`, `b29afa0`, `8f5940f`, `3efa55c`, + epilogue). **Two deliberate deviations, both carried forward:**
  ...
  - **Nothing was deployed *by this slice*.** No hosting deploy, not even a preview channel; the public-URL regression stayed unpublished and App Check enforcement stayed off. **Both were picked up by `S-02`'s Phase 6 on 2026-07-30**, which deployed `S-05b` + `S-01` + `S-02` together and enabled enforcement — so neither is outstanding any more. See Open Roadmap Questions #2 and #6.
...
- **Parallel with:** S-03
- **Blockers:** —
- **Unknowns:** resolved during implementation (2026-07-30).
  - ~~An empty template must apply as a no-op (US-01 acceptance criteria), but "empty" is undefined for fields that carry defaults rather than blanks.~~ → **"Empty" means equal to the defaults, and the two halves of the requirement were separated.** A stored template always carries every key of its domain and Apply always writes all ten, including `null`s — that is FR-011's "replaces all template-controlled fields". Emptiness is a separate, explicit comparison against one `TEMPLATE_DOMAIN_DEFAULTS` constant that mirrors `createForm()`; when a template matches it, Apply does nothing and shows no dialog. Because Apply can also *clear* a field the teacher typed, the confirmation dialog lists fields it will clear alongside fields it will overwrite — FR-011 only names the second.
...
- **Outcome (2026-07-30):** delivered in five phases on `10xdevs-S02` (`f0c08c5`, `8a822b2`, `f386745`, `8551b70`, + this documentation phase). **The north star runs end-to-end**: a teacher fills the cohort fields, saves a named template, sees it listed, applies it to a fresh report behind a confirmation dialog, deletes it, and downloads a PDF indistinguishable from a hand-typed one. Four things are worth carrying forward.
  - **The inherited prerequisite was built first.** The emulator suite, `.emulator-data/`, JDK 21, and the `npm run test:rules` harness landed in Phase 1, *before* the first per-teacher rule — which closes Open Roadmap Question #5 for this slice and hands `S-03` a working harness instead of an obligation. `infrastructure.md` → Getting Started step 4 records what was and was not built.
  - **The field-domain boundary is machine-checked, not documented.** `src/app/templates/template-domain.ts` partitions all 48 controls of the trimester/semester form into four disjoint sets — `TEMPLATE_DOMAIN` (10), `STUDENT_IDENTITY_FIELDS` (4), `PER_STUDENT_FIELDS` (12), `UNREACHABLE_FIELDS` (22) — and `semestr-report.component.spec.ts` asserts pairwise disjointness plus that the union equals `Object.keys(form.controls)` exactly. See the note under `S-04`.
  - **The domain was narrowed below what FR-011 permits.** Every per-student assessment field stays out: a template that pre-filled a mark would turn "the teacher missed one select" into "a parent received another child's grade". `realizedMaterial` and every organizational select are in, so the mechanical bulk of the retyping is still removed.
  - **Rules enforce more than the UI does.** `allow update: if false` on `users/{uid}/reportTemplates/{templateId}`, with the normalized name as the document id, makes PRD's write-once non-goal and the uniqueness of template names one store-level invariant rather than two UI checks.
- **Status:** done — implemented and **deployed 2026-07-30**. Phase 6 ran: Firestore rules released first, then hosting (28 files from `dist/browser`), publishing `S-05b` + `S-01` + `S-02` in one release; App Check enforcement enabled after confirming verified requests. Shipped **without** the FR-004 cutover message, deliberately — see Open Roadmap Question #2. Post-deploy verification against production passed, including a second seeded account seeing only its own templates.
...
- **Inherited from `S-02` (2026-07-30):** a working emulator suite and rules-testing harness. `S-03` no longer owns building them — it owns *using* them: add its own scenarios to `test/rules/` before shipping its first per-teacher rule, and follow the path-keyed ownership shape (`users/{uid}/<collection>/{docId}`, compared against `request.auth.uid`) that `firestore.rules` prints as the template to copy. The gateway/service split in `src/app/templates/` is the seam to mirror for the roster's Firestore calls.
...
- **Outcome (2026-07-31):** delivered in four phases on `10xdevs-S03` (`e6dadbb`, `f0273a7`, `266de58`, + this documentation phase). A teacher opens **Uczniowie / Students** from the header, adds a student, sees their own roster sorted by name, edits any field in place, and deletes behind a confirmation dialog; a second allowlisted account sees a disjoint list and cannot reach the first one's subtree. Five things are worth carrying forward.
  - **The stored record carries FOUR identity fields, not the two the PRD names — a deliberate widening.** `studentName`, `name` (the display first name substituted into PDF prose), `sex`, `class`. `sex` is `Validators.required` on the report form and selects the gendered wording `Uczeń`/`Uczennica` in the PDF; a roster omitting it would leave `S-04`'s picker filling two of four fields and the teacher re-picking sex for every report — the retyping this whole change exists to remove. PRD FR-005 and §Non-Goals carry a dated amendment recording this.
  - **`class` is the same closed select as the report form**, sourced from the same `classes` constant in `src/app/shared/select-values.ts` — thirteen fixed values. FR-005's "free-text class label" does not survive contact with that control: a free-text label pre-filled by `S-04` would be a value outside the option list. The letter of FR-005 was traded for a working `S-04` prefill.
  - **Document ids are Firestore auto-ids, unlike templates.** A student's name is mutable (FR-007) and non-unique — two children may share one — so a name-keyed id would turn a rename into a non-atomic delete + create and would reject a legitimate second "Jan Kowalski". The name is an ordinary field; nothing enforces uniqueness, deliberately.
  - **The project's first `update` rule.** `users/{uid}/students/{studentId}` allows read, create, update and delete to the owning, allowlisted uid — where `reportTemplates` one block up denies `update` to everyone. Document-shape validation is deliberately absent from the rule: a rule restating a schema drifts from it silently, and the boundary that actually protects a child's record is ownership. `test/rules/students.test.mjs` proves all four operations across five callers.
  - **The `S-04` seam is machine-checked, not documented.** `src/app/students/student-domain.spec.ts` asserts `STUDENT_IDENTITY_DOMAIN` names exactly the same four controls as `templates/template-domain.ts`'s `STUDENT_IDENTITY_FIELDS`. If either side drifts, a spec fails instead of the picker silently ceasing to fill a field.
- **What it cost that the plan did not predict:** adding a second file to `test/rules/` broke the first one. `node --test` runs one process per file in parallel against one emulator, and `@firebase/rules-unit-testing` namespaces by `projectId` — so two files sharing an id share a dataset, and each `clearFirestore()` deleted the other's fixtures mid-test. The symptom appeared in the untouched templates suite. Recorded in `context/foundation/lessons.md` and at the bottom of `firestore.rules`.
- **Known consequence, deliberately not solved here:** navigating to `/students` destroys `ShellComponent`, so the active tab resets to Teddy Eddie and any half-filled report form is lost. Tolerable for `S-03` — roster management happens *before* report writing. **Not tolerable for `S-04`**, where "this student isn't in my roster yet" arrives mid-report; that slice needs either an in-form quick-add or preserved form state. Recorded in `src/CLAUDE.md`.
- **Status:** done — implemented and **deployed 2026-07-31**. Phase 5 ran: `10xdevs-S03` fast-forwarded into `10xdevs` (`3c67a3e`), Firestore rules released first, then hosting. Post-deploy verification against production passed, including a second seeded account seeing only its own roster and the four report forms still rendering and downloading a PDF.
...
- **Prerequisites:** S-02, S-03 — **both landed.** This slice is unblocked as of 2026-07-31.
- **Parallel with:** —
- **Blockers:** —
- **Unknowns:** —
- **The roster stores exactly `STUDENT_IDENTITY_FIELDS`, and a spec asserts it (added 2026-07-31).** `S-03` deliberately widened the record from the PRD's two fields to all four — `studentName`, `name`, `sex`, `class` — precisely so this picker can emit a `StudentIdentity` object into the form with **no mapping layer**, the same way `ReportTemplateFields` is the payload `TemplatePanelComponent` emits. `src/app/students/student-domain.spec.ts` asserts `STUDENT_IDENTITY_DOMAIN` and `STUDENT_IDENTITY_FIELDS` name the same four controls, so a drift on either side fails a test rather than silently leaving a field unfilled. Two consequences for this slice: `class` is a value from the `classes` select (not free text), so no value-mapping is needed at the form boundary; and `sex` arrives pre-picked, which is what removes the per-report re-entry the north star's flow still has today.
- **The navigation cost `S-03` deferred lands here (added 2026-07-31).** `/students` is a route, so leaving the shell resets `TabData.tabs` to its default and drops any unsaved report-form state. `S-03` accepted that because roster management happens before report writing. This slice cannot: "the student isn't in my roster yet" arrives *mid-report*, and sending the teacher to `/students` would discard the form they were filling. Budget for either an in-form quick-add or preserved shell state — it is not a detail that resolves itself.
- **The disjoint-domain rule is now machine-checked, and this is the slice that depends on it (added 2026-07-30).** `S-02` declared the partition in `src/app/templates/template-domain.ts` and asserted it in `src/app/semestr-report/semestr-report.component.spec.ts`: `TEMPLATE_DOMAIN` (10), `STUDENT_IDENTITY_FIELDS` (4 — `studentName`, `name`, `sex`, `class`), `PER_STUDENT_FIELDS` (12), `UNREACHABLE_FIELDS` (22), pairwise disjoint, union equal to `Object.keys(form.controls)`. The picker writes only the second set; if it needs a control from any other set, the spec fails and the boundary FR-013 rests on has to be re-decided deliberately rather than drifted through. `S-02` also left the seam to reuse: `SemestrReportComponent` exchanges a typed payload with the panel through two methods and never hands out `form`, and the `date` conversion (`YYYY-MM-DD` at the form boundary, so no other file learns whether the control holds a `Moment` or a `Date`) is the pattern to copy for any date the picker pre-fills. Note the 22 `UNREACHABLE_FIELDS` are controls no template binds — do not treat their presence in the form as a licence to pre-fill them.
...
| S-05b      | `report-design-refresh`      | Align the semester, year-end, and Cambridge forms with the Teddy Eddie design | done                  | Merged 2026-07-29, **deployed 2026-07-30** (in `S-02`'s Phase 6). Year-end PDF fidelity asserted but not captured — Open Roadmap Question #7. |
| S-01       | `google-sign-in-gate`        | Gate the four report forms behind Google sign-in                      | done                  | Merged 2026-07-29, **deployed 2026-07-30** (in `S-02`'s Phase 6) — the FR-004 public-URL regression is now live. Emulator harness deferred to S-02/S-03 and built there. |
| S-02       | `trimester-report-templates` | Save, list, apply, and delete trimester/semester templates            | done                  | North star, implemented and **deployed 2026-07-30**. Phase 6 published `S-05b` + `S-01` + `S-02` in one release and enabled App Check enforcement. Built the emulator suite and rules harness first; ownership keys on the `{uid}` path segment. |
| S-03       | `student-roster`             | Teacher-scoped student roster with add / view / edit / delete         | done                  | Implemented and **deployed 2026-07-31** (rules first, then hosting). Stores four identity fields, not two — PRD FR-005 / §Non-Goals amended to match. First `update` rule in the project. **Minors' data is now live — Open Roadmap Question #1 is no longer hypothetical.** |
| S-04       | `student-picker-in-report`   | Pre-fill student-identity fields from the roster in the report form   | yes                   | **Unblocked** — S-02 and S-03 both landed. Two notes to read first, both under S-04: the roster stores exactly `STUDENT_IDENTITY_FIELDS` (spec-asserted, so no mapping layer), and the shell-reset cost `S-03` deferred lands in this slice. |

## Open Roadmap Questions

1. **GDPR baseline for minors' data. STILL OPEN — and no longer hypothetical as of 2026-07-31.** Export on request, deletion on request, retention windows, and consent-flow language were never pinned. Owner: school director. Block: nothing in MVP (Britannia is the only tenant, and FR-008 provides a deletion path), but it gates any use beyond Britannia. Carried verbatim from PRD Open Question #2.

   **`S-03` shipped, so student data is live.** What is stored per child: `studentName` (full name), `name` (display first name), `sex`, and `class`, at `users/{uid}/students/{studentId}` — one teacher's subtree, unreadable by any other teacher, proven by `test/rules/students.test.mjs`. Note the record is **wider than the PRD's data-minimization claim described** — four fields, not "a name and a free-text class label" — because `sex` drives gendered PDF wording and would otherwise be re-entered per report. That widening is deliberate and recorded as a dated amendment in the PRD, but it is exactly the kind of detail a GDPR conversation needs to start from, so it is repeated here rather than left in the FR. FR-008 (delete a student) remains the only deletion path; there is no export, no retention window, and no consent language. Nothing about this changed the question's owner or its urgency — it changed the question from "what will we store" to "here is what we store".
2. **Cutover communication to bookmarked-link users.** FR-004 removes public-URL access; the message telling existing visitors that they now need a seeded account does not exist. Owner: school director. Carried from PRD Open Question #4. **OVERTAKEN BY EVENTS 2026-07-30 — not answered.** `S-02`'s Phase 6 deployed `S-05b` + `S-01` + `S-02` in one release, so **the FR-004 regression is now live and the message never shipped**. Anyone holding a bookmark to `britannia-reports.web.app` now meets a sign-in screen with no advance notice. That was decided during `S-02`'s planning as a deliberate acceptance of the cost, not discovered afterwards. What remains is no longer a deploy gate but a support question the director still owns: whoever gets asked "why can't I open the reports any more" needs an answer, and seeding that person's address in `allowedUsers` is the fix (`docs/teacher-allowlist-runbook.md`). Closing this as a roadmap question; if the director wants a retroactive note sent, that is new work, not this item.
3. ~~**Backend persistence platform (PRD Open Question #3) — effectively resolved, PRD not yet updated.**~~ **CLOSED 2026-07-30.** The PRD now carries #3 under `## Open Questions` → "Resolved since this PRD was written", with the execution record: Firebase selected, Firestore created in `eur3` by `F-01`, the first collection and conditional rule added by `S-01`, and the first per-teacher collection plus the rules harness added by `S-02`. The two documents agree; nothing is outstanding.
...
5. ~~**Firestore rules have no automated test, and the next slice inherits that.**~~ **CLOSED for `S-02` on 2026-07-30; `S-03` inherits the harness, not the obligation.** `S-01` shipped the first conditional rule (`allowedUsers`) without a harness, deliberately — read-only to every caller, no student data, so the pre-mortem's cross-teacher leak had no surface in it. That argument expired at `S-02`'s first per-teacher collection, and `S-02` built the harness in its Phase 1, **before** writing the rule above it.

   **What exists now:** an `emulators` block in `firebase.json` (Firestore 8080, Auth 9099, UI 4000), `useEmulators: true` in `environment.ts` with `connect*Emulator` wiring in `app.config.ts` and `provideAppCheck` skipped while the flag is on, `npm run emulators` with a gitignored `.emulator-data/`, JDK 21.0.12 on the dev machine, and `npm run test:rules` asserting four scenarios: owner reads/creates/deletes their own template, teacher B denied read/create/delete against it, unauthenticated caller denied everything, and `update` denied even to the owner. No second test *framework* was needed — Node's built-in `node --test` on `.mjs` files outside `src/` keeps Karma, `tsconfig.spec.json` and ESLint out of it.

   **What is still true and worth reading as a residual risk, not a closed one:** the harness is **not** wired into `npm test` and there is no CI, so nothing runs it for you — run it yourself whenever `firestore.rules` changes, before deploying. Owner for that habit: whoever edits the rules. `S-03` owes its own scenarios. Added 2026-07-28, closed 2026-07-30.
6. ~~**App Check enforcement is unowned.**~~ **CLOSED 2026-07-30 — enforcement is ON.** Registered in monitoring mode since F-01; `S-01` was expected to enable it and deliberately did not, since enforcement requires a deployed client and that slice deployed nothing. `S-02`'s Phase 6 ran the sequence: deployed the wired client, used the live app so real attestations flowed, confirmed the verified-request ratio in the Console, *then* enforced — and re-verified sign-in plus a template save afterwards. Enforcing before observing would have locked out every legitimate request had the reCAPTCHA key or the authorized domain been misconfigured.

   **Two things to carry forward rather than close.** A broken App Check surfaces as `permission-denied` **on a Firestore call, not as a sign-in failure** — so a clean sign-in followed by a failing save is this, not a rules bug. And `provideAppCheck` is skipped entirely while `useEmulators` is true, so local development against the emulators never exercises this path; flipping that flag to `false` locally now requires a registered debug token per machine and browser profile, or every Firestore call is denied. Added 2026-07-28, closed 2026-07-30.
