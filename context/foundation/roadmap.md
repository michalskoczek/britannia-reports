...
| S-03       | `student-roster`             | Teacher-scoped student roster with add / view / edit / delete         | yes                   | S-01 and S-02 landed. **The harness now exists — add scenarios to `test/rules/`, don't build it.** Mirror `src/app/templates/`'s gateway/service split and the path-keyed rule shape. First slice to persist minors' data — see Open Roadmap Question #1. |
| S-04       | `student-picker-in-report`   | Pre-fill student-identity fields from the roster in the report form   | no                    | Needs S-03; S-02 landed. The field-domain partition it depends on is now asserted in `semestr-report.component.spec.ts` — read the note under S-04 first. |
