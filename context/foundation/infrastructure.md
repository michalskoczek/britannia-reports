...
| Firestore security rules silently over-permissive — data leakage between teachers (students FR-005..008, templates FR-009..012) | Devil's advocate, Pre-mortem | M | H | Adopt rules-testing harness (`@firebase/rules-unit-testing` + emulator) BEFORE first Firestore deploy. Write per-FR rule test across both collections: teacher A cannot read teacher B's students/templates. Tests in `*.rules.spec.ts`. |
