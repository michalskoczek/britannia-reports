...
- **App Check: registered, MONITORING ONLY (2026-07-23).** The web app is registered with a **reCAPTCHA v3** provider; Firestore enforcement is **off**. Site key `6LfffGEtAAAAAHj0u6_WmNZLrw6FM9BbLukS05ZL` — public by design, it ships in the SPA bundle (it lives in `src/environments/environment*.ts`). Until enforcement is flipped, App Check protects nothing.
  ...
  **Open question — does attestation actually pass on `localhost`?** F-01 wired `provideAppCheck` with a debug-token escape hatch, but the dev boot emitted no App Check warnings and printed no debug token, so none was registered. The likely reason is that reCAPTCHA v3 accepts `localhost` outright. This is **unproven**: F-01 makes zero Firestore and zero Auth calls, so no attestation was ever exercised. `S-01` finds out on its first real request — if it fails there, register a debug token via Firebase Console → App Check → Apps → Manage debug tokens (per machine, per browser profile).
