...
| `.section-row` and its modifiers — the **section**-level row (`S-04`, 2026-08-03) | `semestr-report.component.scss` only | `form-row` / `form-col` lay out fields *inside* one section; this lays whole `<section>` cards out beside each other — student picker / templates / report type as thirds, grade scale beside descriptive grade as an unequal pair. It is built **on** the published mixins rather than beside them, so nothing is hardcoded, and it has exactly one consumer. Left component-local until a second form needs it: promoting a one-consumer idiom to `patterns/` fixes the column ratios before anyone knows whether they generalise. `--grades` is deliberately not expressible through `form-col`, which takes a column *count* and so only produces equal columns; its widths and its collapse are written out. |
